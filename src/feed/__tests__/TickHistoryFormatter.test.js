...
test('Test parse history style="candles"', () => {
    ...
    expect(history)
        .toEqual(historyCandleResponseResult);
});

test('Test parse history style="ticks"', () => {
    const history = TickHistoryFormatter.formatHistory(historyTicksResponse);
    expect(history)
        .toEqual(historyTicksResponseResult);
});

test('Test parse tick style="candles"', () => {
    const tick = TickHistoryFormatter.formatTick(tickCandleResponse);
    expect(tick)
        .toEqual(tickCandleResponseResult);
});

test('Test parse tick style="ticks"', () => {
    const tick = TickHistoryFormatter.formatTick(tickTickResponse);
    expect(tick)
        .toEqual(tickTickResponseResult);
});
