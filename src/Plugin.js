...
(function (definition) {
    ...
    CIQ.iscroll.newScroller = function (node, params) {
        ...
        let iscroll = new IScroll5(node, params);
    ...
    CIQ.iscroll.refresh = function () {
        for (let i = 0; i < CIQ.iscroll.scrollers.length; i++) {
            let iscroll = CIQ.iscroll.scrollers[i];
            iscroll.refresh();
        }
    };
    ...
    CIQ.DialogManager.modalBegin = function () {
        ...
        for (let i = 0; i < CIQ.ChartEngine.registeredContainers.length; i++) {
            let stx = CIQ.ChartEngine.registeredContainers[i].stx;
            stx.modalBegin();
        }
    };
    ...
    CIQ.DialogManager.dismissDialog = function () {
        ...
        let node = CIQ.DialogManager.stack.pop();
        if (!node) return;
