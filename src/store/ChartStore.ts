...
class ChartStore {
    ...
    @action.bound openFullscreen() {
        ...
        // fncToCall is undefined for iOS' Safari that does not support fullscreenAPI
        if (fncToCall) {
            (el as HTMLElement)[fncToCall as 'requestFullscreen']()?.catch(() => { /* no actual handling needed */ });
        }
    }
}
