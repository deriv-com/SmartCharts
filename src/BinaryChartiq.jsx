...
class BinaryChartiq extends Component {
    ...
    componentDidMount() {
        ...
        const resizeScreen = () => {
            if (!UIContext) return;
            updateHeight();
            let sidePanel = $$$(`${this._elementId} cq-side-panel`);
            if (sidePanel) {
                $(`${this._elementId} .ciq-chart-area`)
                    .css({ right: `${sidePanel.nonAnimatedWidth()}px` });
                $(`${this._elementId} cq-tradingcentral`)
        ...
        const retoggleEvents = () => {
            let active = $(`${this._elementId} .stx-markers .ciq-radio.ciq-active`);
            active.parent()
        ...
        const startUI = () => {
            const contextNode = $(`${this._elementId} cq-context,[cq-context]`);
            ...
            const symbolLookup = $$$(`${this._elementId} .ciq-search cq-lookup`);
            ...
            const self = this;
            $(`${this._elementId} .ciq-draw`)[0].registerCallback(function (value) {
                if (value) {
                    this.node.addClass('active');
                    CIQ.appendClassName($$$(`${self._elementId} .ciq-chart`), 'toolbar-on');
                } else {
                    this.node.removeClass('active');
                    CIQ.unappendClassName($$$(`${self._elementId} .ciq-chart`), 'toolbar-on');
                }
                updateHeight();
                stxx.resizeChart();
            ...
            $(`${this._elementId} cq-redo`)[0].pairUp($(`${this._elementId} cq-undo`));
            ...
            let UIStudyMenu = new CIQ.UI.StudyMenu($(`${this._elementId} *[cq-studies]`), UIContext, params);
