function browserDetect(rootEleId, SupportedBrowsers) {
    ...
    function renderNotSupported() {
        const cqNotSupported = document.createElement('div');
        cqNotSupported.style.cssText =
            'background:#fff url("./dist/app/assets/bg-not-supported.svg") repeat scroll center center;position:fixed;width:100%;height:100%;font-family:"Roboto";font-style:normal;line-height:1.5;letter-spacing:normal;text-align:center;color:#333333;';
        const cqLogo = document.createElement('div');
        cqLogo.style.cssText =
            'background:url("./dist/app/assets/binary-logo.svg") no-repeat scroll center center;width:139px;height:32px;margin-bottom:8%;padding:20px;';
        cqNotSupported.appendChild(cqLogo);
        const cqIconNotSupported = document.createElement('div');
        cqIconNotSupported.style.cssText =
            'background:url("./dist/app/assets/icon-not-supported.svg") no-repeat scroll center center / 160px 160px;width:160px;height:160px;margin:auto;';
        cqNotSupported.appendChild(cqIconNotSupported);
        const cqH1 = document.createElement('h1');
        ...
        const cqP = document.createElement('p');
        ...
        const cqA = document.createElement('a');
    ...
    if (browser) {
        const version = parseInt(browser.version.split('.').join(''));
        const name = browser.name;
        const os = browser.os;
        if (SupportedBrowsers[os] && SupportedBrowsers[os][name] !== undefined) {
