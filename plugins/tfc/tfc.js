...
CIQ.TFC.prototype.dragOrdersHeader = function (e) {
    let header = this.elements.openOrdersHeader;
...
CIQ.TFC.prototype.getCurrentPriceForOrder = function (activeTrade) {
    let currentQuote = this.stx.currentQuote();
    let bid = null,
        ask = null;
    if (currentQuote) {
...
CIQ.TFC.prototype.enableBuy = function (params) {
    ...
    if (params && params.openOrder) {
        ...
        if (params.openOrder.oto) {
            for (let oto = 0; oto < params.openOrder.oto.length; oto++) {
...
CIQ.TFC.prototype.enableSell = function (params) {
    ...
    CIQ.unappendClassName(this.dom.limitOrder, 'with-limit');
    let quantity = 0;
    let position = this.account.positions[this.stx.chart.symbol];
...
CIQ.TFC.prototype.updateValues = function () {
    ...
    if (this.activeTrade == 'bracket_sell' || this.activeTrade == 'bracket_cover') {
        if (this.elements.bracketId.value) {
            let trades = this.account.trades[this.stx.chart.symbol];
    ...
    let lossBracketDifferential = null;
    let profitBracketDifferential = null;
    if (this.activeTrade == 'market') {
        ...
        if (this.account.isForex(this.stx.chart.symbol)) {
            ...
            let wholePart = str.substring(0, str.indexOf('.'));
            if (wholePart === '') wholePart = '0';
            let decimalPart = str.substring(str.indexOf('.') + 1);
            let equityPart = `${wholePart}.${decimalPart.substring(0, 2)}`;
            let strong = decimalPart.substring(2, 4);
            let sub = decimalPart.substring(4, 5);
    ...
    let gainLoss,
        ...
        reward,
        ratio;
    let abovePrice = this.quantityFromValue(this.elements.dragLineAbovePrice.innerHTML);
    let belowPrice = this.quantityFromValue(this.elements.dragLineBelowPrice.innerHTML);
    ...
    let e;
    let tradesLikeForex = this.account.tradesLikeForex(this.stx.chart.symbol);
...
CIQ.TFC.prototype.clearEphemeral = function (which) {
    let nodes = this.ephemeralNodes[which];
    for (let i = 0; i < nodes.length; i++) {
        let node = nodes[i];
...
CIQ.TFC.prototype.instantiateTemplate = function (template, which) {
    let node = this.templates[template].cloneNode(true);
...
CIQ.TFC.prototype.modifyOpenOrder = function (openOrderMarker) {
    let openOrderMarkers = this.ephemeralNodes.openOrders;
    // var position=this.account.positions[this.stx.chart.symbol];
    // var quantity=0;
    // if(position) quantity=position.quantity;
    let openOrder = openOrderMarker.openOrder;
    ...
    if (openOrder.action == 'buy') {
        this.newTrade('enableBuy', { openOrder });
    } else if (openOrder.action == 'cover') {
        this.newTrade('enableCover', { openOrder });
    } else if (openOrder.action == 'sell') {
        this.newTrade('enableSell', { openOrder });
    } else if (openOrder.action == 'short') {
        this.newTrade('enableShort', { openOrder });
    }
    if (openOrder.oto) {	// Enable and position the OTO elements, if any
        ...
        for (let i = 0; i < openOrder.oto.length; i++) {
            let order = openOrder.oto[i];
            if (order.stop) {
...
CIQ.TFC.prototype.overlap = function (a, b) {
    let t1 = CIQ.stripPX(a.style.top);
    let b1 = t1 + a.offsetHeight;
    let t2 = CIQ.stripPX(b.style.top);
...
CIQ.TFC.prototype.renderTrades = function () {
    ...
    if (trades && trades.length > 0) {
        for (let i = 0; i < trades.length; i++) {
            let trade = trades[i];
            let panel = this.stx.panels[this.chart.name];
            this.stx.startClip(panel.name);
            let t0 = this.stx.tickFromDate(CIQ.yyyymmddhhmm(new Date(trade.time)));
            let x0 = this.stx.pixelFromTick(t0);
            let x1 = this.stx.pixelFromTick(this.stx.chart.dataSet.length);
            let y = this.stx.pixelFromValueAdjusted(panel, t0, trade.basis);
            let style = 'stx-trade-position-line-down';
            ...
            if (this.chart.right > x0) {
                ...
                if (y < this.chart.bottom && y > this.chart.top) {
                    this.stx.endClip();
                    let txt = Number(trade.basis);
                    if (panel.chart.transformFunc) txt = panel.chart.transformFunc(this.stx, panel.chart, txt);
                    if (panel.yAxis.priceFormatter) { txt = panel.yAxis.priceFormatter(this.stx, panel, txt); } else { txt = this.stx.formatYAxisPrice(txt, panel); }
...
CIQ.TFC.prototype.renderOpenOrders = function () {
    ...
    for (i = 0; i < openOrderMarkers.length; i++) {
        ...
        let overlapOffset = 0;
        for (let j = i + 1; j < openOrderMarkers.length; j++) {
            let potentialOverlap = openOrderMarkers[j];
    ...
    for (i = 0; i < openOrderMarkers.length; i++) {
        ...
        if (this.modifyingOrder && this.modifyingOrder.id == openOrderMarker.openOrder.id) {
            ...
            if (openOrderMarker.linked) {
                for (let k = 0; k < openOrderMarker.linked.length; k++) { openOrderMarker.linked[k].style.display = 'none'; }
            }
        }
    }
};
...
CIQ.TFC.prototype.createOpenOrderMarker = function (openOrder, baseOrderMarker) {
    let openOrderMarker = this.instantiateTemplate('openOrderMarker', 'openOrders');
    openOrderMarker.openOrder = openOrder;
    let priceNode = $$$('.tfc-price', openOrderMarker);
    ...
    let whichMarker = openOrderMarker;
    if (baseOrderMarker) {
...
CIQ.TFC.prototype.deriveOpenOrderMarkers = function () {
    ...
    let openOrders = this.account.openOrders[this.stx.chart.symbol];
...
CIQ.TFC.prototype.updateData = function () {
    ...
    function addProtection(t) {
        return function () {
            self.clearActive();
            let componentName = 'enableBracket';
            for (let n = 0; n < self.menu[componentName].nodes.length; n++) {
    ...
    function update() {
        ...
        let position = self.account.positions[self.stx.chart.symbol];
        ...
        let table = $$$('.stx-current-orders tbody');
        CIQ.clearNode(table);
        let foundOne = false;
        let symbolList = [];
        ...
        let foundCurrentSymbol = false;
        let dividerDrawn = false;
        let lastWasOco = false;
        let contractSize,
            i,
            j;
        let tr,
        ...
        for (j = 0; j < symbolList.length; j++) {
            var price;
            symbol = symbolList[j];
            let openOrders = self.account.openOrders[symbol];
            ...
            for (i = 0; i < openOrders.length; i++) {
                ...
                if (lastWasOco) {
                    ...
                    tr.addEventListener('tap', closure({
                        symbol, tfc: self, type: 'order', obj: openOrder,
                    }));
                ...
                if (openOrder.oto instanceof Array) {
                    for (let oto = 0; oto < openOrder.oto.length; oto++) {
                        tr = CIQ.newChild(table, 'TR');
        ...
        let viewType = self.positionsView;
        ...
        for (j = 0; j < symbolList.length; j++) {
            ...
            foundOne = true;
            let currency = CIQ.convertCurrencyCode(position.currency);
            if (foundCurrentSymbol || symbol == self.stx.chart.symbol) {
                if (j) {
                    tr = CIQ.newChild(table, 'TR');
                    tr.className = 'stx-divider';
            ...
            if (!position.securityType || position.securityType.toUpperCase() != 'OPTION') {
                tr.addEventListener('tap', closure({
                    symbol, tfc: self, type: 'position', obj: position,
                }));
            }
            if (viewType == 'lots') {
                ...
                } else if (position.basis > position.price) CIQ.appendClassName(td, 'up');
                else if (position.basis < position.price) CIQ.appendClassName(td, 'down');
            ...
            if (self.account.trades[symbol]) {
                let sumBasis = 0;
                let sumValue = 0;
                for (let t = 0; t < self.account.trades[symbol].length; t++) {
                    let trade = self.account.trades[symbol][t];
                        ...
                        if (viewType == 'maintenance') {
                            ...
                            if (trade.protect) {
                                if (trade.protect.limit) {
                                    let tp = CIQ.newChild(td, 'SPAN', null, `\u00a0@TP\u00a0${trade.protect.limit}`);
                                    tp.style.color = 'green';
                                    protection = true;
                                }
                                if (trade.protect.stop) {
                                    let sl = CIQ.newChild(td, 'SPAN', null, `\u00a0@SL\u00a0${trade.protect.stop}`);
                                    sl.style.color = 'red';
                                    protection = true;
                                }
                            }
                            ...
                            } else if (trade.basis > trade.price) CIQ.appendClassName(td, 'up');
                            else if (trade.basis < trade.price) CIQ.appendClassName(td, 'down');
        ...
        if (!foundOne) {
            ...
            for (let tt in self.account.trades) {
...
CIQ.TFC.prototype.refreshScrollWindows = function (openOrderTop) {
    let positionWrapper = $$$('.stx-current-position tbody').parentNode.parentNode;
    let openOrderWrapper = $$$('.stx-current-orders tbody').parentNode.parentNode;
    let toggle = $$$('.stx-trade-ticket-toggle.close');
    let totalHeight = positionWrapper.parentNode.parentNode.clientHeight - positionWrapper.parentNode.offsetTop - toggle.clientHeight;
    let positionHeight,
        openOrderHeight;
    ...
    for (symbol in this.account.openOrders) {
        let openOrder = this.account.openOrders[symbol];
    ...
    if (openOrderTop) {
        ...
        let ratio = positionCount / (positionCount + openOrderCount);
...
CIQ.TFC.prototype.configureMenu = function () {
    ...
    let symbol = self.stx.chart.symbol;
    this.account.tradability(symbol, (tradability) => {
        self.tradability = tradability;
        if (!symbol) tradability.tradable = false; // make sure cannot trade when no security enabled

        let position = self.account.positions[symbol];
        let hasShort = position && position.quantity < 0;
        let hasLong = position && position.quantity > 0;
        // hedging check
        let trades = self.account.trades[symbol];
        for (let t = 0; trades && t < trades.length; t++) {
        ...
        if (self.account.config.reducePosition === false) {
            let orders = self.account.openOrders[symbol];
            for (let o = 0; orders && o < orders.length; o++) {
...
CIQ.TFC.prototype.closeTFC = function () {
    ...
    let wsInTicks = Math.round(this.stx.preferences.whitespace / this.stx.layout.candleWidth);
...
CIQ.TFC.prototype.newTrade = function (componentName, params) {
    ...
    this.hideAllDOM();
    let which = this.menu[componentName];
    console.warn(which);
    let dom = which.dom;
    for (let i = 0; i < dom.length; i++) {
        let tradeElementName = dom[i];
        this.dom[tradeElementName].style.display = '';
    }
    this.crosshairsOriginallyOn = this.stx.layout.crosshair;
    ...
    if (componentName != 'enableMarket') {
        let wsInTicks = Math.round(this.width / this.stx.layout.candleWidth);
        this.stx.chart.scroll = this.stx.chart.maxTicks - wsInTicks;
        this.stx.draw();
    }
    this.updateValues();
...
CIQ.TFC.prototype.clearActive = function () {
    for (let componentName in this.menu) {
        let components = this.menu[componentName].nodes;
...
CIQ.TFC.prototype.placeOrder = function (order) {
    ...
    if (order.type == 'close') {
        let trades = this.account.trades[order.symbol];
        if (trades && order.id) {
            for (let t = 0; t < trades.length; t++) {
            ...
            let position = this.account.positions[order.symbol];
            let err = 'No matching position found.';
            if (position) {
        ...
        this.account.replaceOrder(this, order, (err, obj) => {
            CIQ.DialogManager.dismissDialog();
            if (err) {
                CIQ.alert(err);
            }
        ...
        this.account.setProtection(this, order, (err, obj) => {
            CIQ.DialogManager.dismissDialog();
            if (err) {
                CIQ.alert(err);
            }
        ...
        this.account.placeOrder(this, order, (err, obj) => {
            CIQ.DialogManager.dismissDialog();
            if (err) {
                CIQ.alert(err);
            }
...
CIQ.TFC.prototype.cancelOpenOrder = function () {
    ...
    if (self.config.skipConfirms) {
        self.account.cancelOrder(self, order, (err) => {
    ...
    this.account.confirmOrder(this, order, (confirmation) => {
        CIQ.DialogManager.displayDialog('tfcConfirmOrder');
        let descriptionNode = $$$('#tfcConfirmOrder .tfcOrderDescription');
        let priceNode = $$$('#tfcConfirmOrder .tfcOrderPrice');
        let tifNode = $$$('#tfcConfirmOrder .tfcOrderTif');
        let otoNode = $$$('#tfcConfirmOrder .tfc-confirm-oto');
        let basisNode = $$$('#tfcConfirmOrder .tfc-confirm-basis');
        let estPlNode = $$$('#tfcConfirmOrder .tfc-confirm-pl');
        basisNode.style.display = 'none';
        estPlNode.style.display = 'none';

        let actionText = `CANCEL ${order.action.capitalize()}`;
        if (order.quantity <= 0) {
            if (order.action == 'sell') actionText += ' (short)';
            else actionText += ' (to cover)';
        }
        let description = `${actionText} ${CIQ.commas(order.quantity)}`;
        ...
        let submitButton = $$$('#tfcConfirmOrder .tfcSubmit');
        let abandonButton = $$$('#tfcConfirmOrder .tfcAbandon');
...
CIQ.TFC.prototype.createOCOFromGUI = function () {
    ...
    let arr = [];
    if (this.activeTrade == 'straddle' || this.activeTrade == 'strangle' || this.dom.otoAbove.style.display != 'none') { arr.push(order); }
    if (this.activeTrade == 'straddle' || this.activeTrade == 'strangle' || this.dom.otoBelow.style.display != 'none') { arr.push(order2); }
    return arr;
};
...
CIQ.TFC.prototype.quantityFromValue = function (value) {
    let quantity = parseFloat(value.replace(/,/g, ''));
...
CIQ.TFC.prototype.createOrderFromGUI = function (action) {
    ...
    if (action == 'limit_buy' || action == 'limit_sell' || action == 'limit_short' || action == 'limit_cover') {
        order.quantity = this.quantityFromValue(this.elements.limitShares.value);
        order.tif = this.elements.limitTIF.value;
        let currentPrice = this.getCurrentPriceForOrder(order.action);
        let centerPrice = this.quantityFromValue(this.elements.dragLineCenterPrice.innerHTML);
        if (!this.limitFlippedToMarket) {
            if (order.action == 'buy' || order.action == 'cover') {
                if (centerPrice <= currentPrice) order.limit = centerPrice;
                else order.stop = centerPrice;
            } else if (centerPrice >= currentPrice) order.limit = centerPrice;
            else order.stop = centerPrice;
        }
        ...
        let currentQuote = this.stx.currentQuote();
        let bid = currentQuote.Bid ? currentQuote.Bid : currentQuote.Close;
        let ask = currentQuote.Ask ? currentQuote.Ask : currentQuote.Close;
        ...
        if (this.account.config.oto) {
            ...
            let loss = Number(this.elements.marketLossBracketDifferential.value) / Math.pow(10, decimalPlaces);
            let profit = Number(this.elements.marketProfitBracketDifferential.value) / Math.pow(10, decimalPlaces);
            if (loss > 0) {
    ...
    if (this.dom.otoAbove.style.display != 'none' || this.dom.otoBelow.style.display != 'none') {
        let oto = this.createOCOFromGUI();					// re-use existing code to create OCO
...
CIQ.TFC.prototype.createReplaceFromGUI = function () {
    ...
    } else if (centerPrice >= currentPrice) order.limit.new = centerPrice;
    else order.stop.new = centerPrice;
...
CIQ.TFC.prototype.confirmReplace = function (order) {
    let descriptionNode = $$$('#tfcConfirmReplace .tfcOrderDescription');
    let quantityNodeOld = $$$('#tfcConfirmReplace .tfcOrderQuantityOld');
    let priceNodeOld = $$$('#tfcConfirmReplace .tfcOrderPriceOld');
    let tifNodeOld = $$$('#tfcConfirmReplace .tfcOrderTifOld');
    let otoNodeOld = $$$('#tfcConfirmReplace .tfcOrderOTOOld');
    let quantityNodeNew = $$$('#tfcConfirmReplace .tfcOrderQuantityNew');
    let priceNodeNew = $$$('#tfcConfirmReplace .tfcOrderPriceNew');
    let tifNodeNew = $$$('#tfcConfirmReplace .tfcOrderTifNew');
    let otoNodeNew = $$$('#tfcConfirmReplace .tfcOrderOTONew');
    let otoLine = $$$('#tfcConfirmReplace .tfc-confirm-oto');

    let actionText = order.action;
    let description = actionText.capitalize();
    ...
    let submitButton = $$$('#tfcConfirmReplace .tfcSubmit');
    let abandonButton = $$$('#tfcConfirmReplace .tfcAbandon');
    function closure(self, order, submitButton) {
        return function () {
            CIQ.clearSafeClickTouches(submitButton);
            CIQ.clearSafeClickTouches(abandonButton);
...
CIQ.TFC.prototype.confirmOrder = function (order) {
    ...
    this.account.confirmOrder(this, order, (confirmation) => {
        ...
        if (typeof order.quantity === 'undefined') {
            ...
            let divVsp = document.createElement('div');
...
CIQ.TFC.prototype.confirmCloseTrade = function (trade) {
    let contractSize = trade.contractSize ? trade.contractSize : 1;
    let order = {
        type: 'close',
        id: trade.id,
        action: 'close',
...
CIQ.TFC.prototype.confirmClosePosition = function (position) {
    ...
    if (this.account.trades) {
        let sumBasis = 0,
            sumValue = 0;
        for (let t = 0; t < this.account.trades[position.symbol].length; t++) {
            let trade = this.account.trades[position.symbol][t];
            sumBasis += (trade.basis * trade.quantity * contractSize);
            sumValue += (trade.price * trade.quantity * contractSize);
        }
...
CIQ.TFC.prototype.confirmCloseAllPositions = function () {
    ...
    let submitButton = $$$('#tfcConfirmCloseAll .tfcSubmit');
    let abandonButton = $$$('#tfcConfirmCloseAll .tfcAbandon');
    function closure() {
        CIQ.clearSafeClickTouches(submitButton);
        CIQ.clearSafeClickTouches(abandonButton);
        $$$('#tfcConfirmCloseAll .processOrder').style.display = 'block';
        CIQ.appendClassName($$$('#tfcConfirmCloseAll'), 'tfc-pending');
        self.account.closeAllPositions(self, (err) => {
            CIQ.DialogManager.dismissDialog();
            if (err) CIQ.alert(err);
            self.updateData();
            self.stx.draw();
...
CIQ.TFC.prototype.confirmOCO = function (order, type) {
    ...
    let tifNode = $$$('#tfcConfirmOCO .tfcOrderTif');
    let headerNode = $$$('#tfcConfirmOCO h4');
    let orderLabels = document.querySelectorAll('#tfcConfirmOCO .stx-label');
    ...
    for (i = 0; i < order.length; i++) {
        let oco = order[i];
        let description = `${oco.action} ${CIQ.commas(oco.quantity)}`;
    ...
    let submitButton = $$$('#tfcConfirmOCO .tfcSubmit');
    let abandonButton = $$$('#tfcConfirmOCO .tfcAbandon');
    function closure(self, order, submitButton) {
        return function () {
            CIQ.clearSafeClickTouches(submitButton);
            CIQ.clearSafeClickTouches(abandonButton);
...
CIQ.TFC.prototype.userAction = function (action) {
    ...
    } else if (this.activeTrade == 'bracket_cover' || this.activeTrade == 'bracket_sell') {
...
CIQ.TFC.prototype.dragMarketOrder = function (e) {
    let newPosition = this.initialPosition + e.displacementY;
    if (newPosition < 0) newPosition = 0;
    let parentNode = this.dom.marketOrder.parentNode;
    ...
    function f(self, componentName) {
        return function (e) {
            ...
            if (self.menu[componentName].nodes[0].className.indexOf('active') == -1) {
                self.clearActive();
                for (let i = 0; i < self.menu[componentName].nodes.length; i++) {
...
CIQ.TFC.prototype.changePositionsView = function (viewType) {
    let nodes = document.querySelectorAll('.tfc-positions-view');
...
CIQ.TFC.prototype.construct = function (config) {
    ...
    sharedParams = { safety };	// since addOTOStop and addOTOLimit are overlapping, on touch devices the touch event will trigger the first
    ...
    CIQ.safeClickTouch(this.elements.limitShares, (function (self) {
        return function (e) {
            if (CIQ.isIOS7or8) {
            // e.preventDefault();
            // e.target.focus();
    ...
    CIQ.safeClickTouch(this.elements.limitCurrency, (function (self) {
        return function (e) {
            if (CIQ.isIOS7or8) {
            // e.preventDefault();
            // e.target.focus();
    ...
    CIQ.safeClickTouch(this.elements.marketShares, (function (self) {
        return function (e) {
            if (CIQ.isIOS7or8) {
            // e.preventDefault();
            // e.target.focus();
    ...
    CIQ.safeClickTouch(this.elements.marketCurrency, (function (self) {
        return function (e) {
            if (CIQ.isIOS7or8) {
            // e.preventDefault();
            // e.target.focus();
    ...
    CIQ.safeClickTouch(this.elements.marketLossBracketDifferential, (function (self) {
        return function (e) {
            if (CIQ.isIOS7or8) {
                e.preventDefault();
                e.target.focus();
    ...
    CIQ.safeClickTouch(this.elements.marketProfitBracketDifferential, (function (self) {
        return function (e) {
            if (CIQ.isIOS7or8) {
                e.preventDefault();
                e.target.focus();
