...
| initialData               | Set initial data that library require for booting up. Refer [initialData](#initial-data) for usage details.                                                                                                                                                                                                                                                      |
| feedCall                  | Enbale/Disable the feed call for getting requirement resource. Default is `{activeSymbols: true,tradingTimes: true}`                                                                                                                                                                                                                                             |
...
| --- | --- | --- |
| activeSymbols | Array of active symbols (available markets) that is used to load market selector. Default is `null`. This value would update in chart is user toggle property of `refreshActiveSymbols` that cause the chart to request for activeSymbols on the Feed | `[{ allow_forward_starting: 0, display_name: 'AUD Index', exchange_is_open: 1, is_trading_suspended: 0, market: 'forex', market_display_name: 'Forex', pip: 0.001, submarket: 'smart_fx', submarket_display_name: 'Smart FX', symbol: 'WLDAUD', symbol_type: 'smart_fx' }, ...]`
|tradingTimes | Array of markets trading time that is used to determine close/open markets. Default is `null`. The chart will request for new data via Feed in a sequence that is calculated regards markets trading time. | `{trading_times: {markets: [{ name: 'Forex', submarkets: [{ name: 'Major Pairs', symbols: [{name: 'AUD/JPY', symbol: 'frxAUDJPY', times: { close: ['23:59:59'], open: ['00:00:00'], settlement: '23:59:59' }, trading_days: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri'],events: [{ dates: 'Fridays', descrip: 'Closes early (at 20:55)' },{ dates: '2020-12-25', descrip: 'Christmas Day' },{ dates: '2021-01-01', descrip: "New Year's Day" }], ...}, ]},...]...],}}`
| masterData | Array of ticks that is used to load the graph (candles). Default is `null`. If the Feed is avilable, the chart will call `fetchInitialData` via Feed to get the the inital ticks, for old data it call `fetchPaginationData` and recieve new ticks constantly. If this property filled, chart user `symbol` property and `setting` property (to extract chart type and interval) and the masterData to load the graph. **notice:** chart interval in the setting property should be same as masterData epoch/Date property. If the symbol property does not fill, the chart use `1HZ10V` as default for initial load | `[{"Date":"2020-11-16T04:28:00", "Close":8287.85}, {"Date":"2020-11-16T04:26:00", "Open":8283.25,"High":8293.750015,"Low":8278.75,"Close":8293.75},...]`
